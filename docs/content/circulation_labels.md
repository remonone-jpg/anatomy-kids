...
소화와 동일한 `DiagramLabel` 스키마.
범례 2개(`a. = 동맥`, `v. = 정맥`)는 클릭 대상에서 제외.

## 원칙

1. 2~3문장. 소화와 같은 톤
2. **어디를 지나 어디에 피를 대는지**를 먼저 말함
3. 몸에서 느낄 수 있는 것은 함께 적음 (맥박 짚는 자리, 채혈하는 자리 등)
4. 순환 기관 `organs`에 있는 것은 심장·혈관·혈액 3개뿐이라
   나머지는 전부 `beyond: true`
5. 이름의 뜻을 풀어 줌 — "노동맥"의 노가 아래팔 바깥쪽 뼈 이름이라는 식

---

```ts
// app/i18n/school/diagrams/circulation-labels-ko.ts
...
export const circulationLabels: DiagramLabel[] = [
  ...
  { id: "aortic-arch", label: "활동맥", name: "대동맥활", beyond: true,
    desc: "대동맥이 심장에서 나와 활처럼 휘어지는 부분이에요. 여기서 머리와 팔로 가는 굵은 가지가 갈라져 나갑니다." },
...
원본의 `a.`(artery)와 `v.`(vein) 약어를 풀어 둔 안내이므로 설명이 필요 없습니다.
...
```
순환 도해 라벨 48개 설명을 넣고 인터랙티브하게 만들어줘.
docs/content/circulation_labels.md 기준. 소화와 같은 방식이야.

[1. 데이터]
app/i18n/school/diagrams/circulation-labels-ko.ts 신규 생성.
DiagramLabel 타입은 소화와 동일.

[2. data-organ 심기]
annotate.py 를 재사용해서 circulation.svg 에 id를 심어줘.
용어 사전만 바꾸면 된다고 했지.

★범례 2개(a. = 동맥, v. = 정맥)는 id를 붙이지 말 것.
  설명이 필요 없는 안내 문구야.

★빈 <text> 4개도 제외.

심고 나서 매칭 목록을 보여주고 잠깐 멈춰줘. 소화 때처럼
엉뚱한 라벨에 엉뚱한 id가 붙으면 나중에 찾기 어려워.

[3. 연결]
소화에서 만든 DiagramViewer 를 그대로 재사용.
순환 기관 화면에서도 "크게 보고 눌러보기"가 뜨게.

[4. 확인]
- 48개 전부 클릭되는지
- 범례 2개는 클릭 안 되는지
- 엉뚱한 설명이 뜨지 않는지
- 연속 클릭이 되는지 (소화에서 고친 그 버그)
