...
| A leader line in the respiratory diagram, from the middle of the left lung | drawn, `#b2b2b2` at 1.91 units | hidden | It points at nothing and joins nothing — the guides that actually reach the alveolar inset start elsewhere and are unaffected. Almost certainly a leftover from the author's own edits, though there is no way to confirm that. It carries no id and matches the working guides in colour and weight, so it is identified by where it starts, and that rule is applied to this file alone. |
