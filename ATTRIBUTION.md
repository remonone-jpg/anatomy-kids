...
| Four grey lines in the respiratory diagram | drawn, `#b2b2b2` at 1.8–1.9 units | hidden | The inset is joined to the drawing by one black line ending in an arrowhead; these four are grey, carry no arrow, and start and end on nothing — one out of the left lung, one past the inset's edge into the lungs, and two forming a run from the top of the sheet to a point beside the arrow without meeting it. Three of them cut across labels. **What the author meant by them could not be established**, but they connect to no element in the file, so they are hidden here. The file keeps them, and removing the rule brings them back. |

One thing is moved: the white cards are drawn just after the box each sits in, so each card paints over its own box's border and leaves the group looking open on one side. The cards are put in front of the boxes instead — still above the artwork, so they go on keeping the drawing off the letters. Beyond that and the line hidden above, nothing is removed or redrawn; only these fills and two stroke widths change. The respiratory diagram's leader lines are left exactly as drawn — the author uses one dark red for both the general leaders and the pulmonary vessels, so there is no way to soften the first without also changing the second.
